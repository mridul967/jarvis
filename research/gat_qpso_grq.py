...
Quantum-inspired CVRP solver with GAT attention, QPSO, and OR-Tools benchmark.
Deps managed via pyproject.toml — run `uv sync` before executing.

Original Colab:
    https://colab.research.google.com/drive/1i74jcATcTKi8eGJnh62bXKGIHRC3MLpq
"""
...
# ponytail: prevent JAX from pre-allocating all GPU VRAM so PyTorch can coexist
os.environ.setdefault("XLA_PYTHON_CLIENT_PREALLOCATE", "false")

# JAX Ecosystem (import AFTER setting env var)
import jax
...
# Resolve dataset path: CLI argument -> local directory -> data/ directory
candidates = [
    sys.argv[1] if len(sys.argv) > 1 else None,
    os.path.join(os.path.dirname(__file__), "X-n1001-k43.xml"),
    "X-n1001-k43.xml",
    os.path.join(os.path.dirname(__file__), "data", "X-n1001-k43.xml"),
]
DATA_FILE = next((p for p in candidates if p and os.path.isfile(p)), "X-n1001-k43.xml")
if not os.path.isfile(DATA_FILE):
    sys.exit(f"ERROR: dataset not found at {DATA_FILE}\n  Usage: python {sys.argv[0]} [path-to-xml]")
...
class Quantum_CVRP_Engine:
    ...
    def solve(self, deadline=55.0):
        ...
        for it in range(self.iters):
            ...
            # ponytail: JAX @jit returns immutable jax arrays; convert back to
            # numpy so the per-particle mutation loop below can do self.pos[i] = ...
